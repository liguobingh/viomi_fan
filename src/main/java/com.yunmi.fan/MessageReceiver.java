...
import com.yunmi.fan.activity.MainActivity;
...
public class MessageReceiver implements IXmPluginMessageReceiver {
    ...
    @Override
    public boolean handleMessage(Context context, XmPluginPackage xmPluginPackage, int type, Intent intent, DeviceStat deviceStat) {
        ...
        switch (type) {
            ...
            case PUSH_MESSAGE: {
                ...
                if ("DevicePush".equals(msgType)) {// 订阅的消息
                    ...
                    UMFanDevice device = UMFanDevice.getDevice(deviceStat);
                    ...
                    Log.d(UMFanDevice.MODEL, "ScenePush :" + event + "  " + extra);
                    //TODO 处理场景通知
                }
            }
            default:
    ...
    @Override
    public boolean handleMessage(Context context, XmPluginPackage xmPluginPackage, int type, Intent intent, DeviceStat deviceStat, MessageCallback callback) {
        //TODO 主app调用插件获取数据
        return false;
    }
